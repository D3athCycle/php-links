...
def userApproval () {
	...
	timeout(time: 5, unit: 'MINUTES'){
		try {
			input message: 'Is this version ready ? In 5 Minutes this step will be processed automatically!', id: 'input1'
		} catch (err) {
			sh "ocp/cleanup.sh"
...
def promoteQA () {
	...
	openshiftDeploy(depCfg: 'app-qa', namespace: 'bananaphp', verbose: 'false', waitTime: '10', waitUnit: 'min')
...
def promotePROD () {
	...
	openshiftDeploy(depCfg: 'app-prod', namespace: 'bananaphp', verbose: 'false', waitTime: '10', waitUnit: 'min')
